...
# Models to choose from [resnet, alexnet, vgg, squeezenet, densenet, efficientnet, convnext]
...
use_pretrained=True
# dataset_name_list = ['2009', '2011', '2013', '2015']
T_list = [20]
loss_list = [(0.3,0.7)]
    ...
    for loss_div in loss_list:
        ...
        if mode =='Train':
            ...
            for epoch in tqdm(range(num_epochs)):
                ...
                pred_labelsv=[]
                with torch.no_grad():
                    for batch_index,(images,labels) in tqdm(enumerate(val_loader)):
                        images = images.to(device)
                        labels = labels.to(device)
                ...
                if f1v > best_f1:
                    ...
                    torch.save(model, f'{model_save_path}/Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}.pt')
            ...
            filename =  '/DATA/data/fold_results/du_mu/alexnet/models_fold_6/model_fold_6.pt'
