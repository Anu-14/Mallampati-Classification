    ...
    if mode =='Train':
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            for images,labels in train_dataset:
                ...
                for i in range(T_monte_carlo):
                    logit_variance_tmp, logits_tmp, variance_tmp, variance_pre_tmp = aleatoric_model(images)
                    logit_variance_list.append(logit_variance_tmp)
                    logits_list.append(logits_tmp)
                    variance_list.append(variance_tmp)
                    variance_pre_list.append(variance_pre_tmp)

                loss_sum = sum(logits_list)

                for i in range(len(logits_list)):
                    weights_for_loss.append((logits_list[i]) / (loss_sum))
                    weighted_loss.append(logits_list[i])
                    sum_loss += weighted_loss[i]
                    
                logits = sum_loss / len(logits_list)
                logit_variance = sum(logit_variance_list) / len(logit_variance_list)
                variance = sum(variance_list) / len(variance_list)
                variance_pre = sum(variance_pre_list) / len(variance_pre_list)
            ...
            if f1v > best_f1:
                ...
                model_save_path = "models_output_du_mu"
            ...
            results_save_path = f"model_result_du_mu/results_____Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}/{model_name}_cropped"
        ...
        print(report)
        print(cf)
        report_path = f"model_result_du_mu/results_____Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}/{model_name}_cropped/classification_report.txt"
        with open(report_path, "w") as report_file:
            report_file.write(report)

        report_file.close()

        cf_path = f"model_result_du_mu/results_____Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}/{model_name}_cropped/cf_report.csv"
        confusion_df = pd.DataFrame(cf)

        confusion_df.to_csv(cf_path)
    
    model_save_path = f"fold_results/du_mu/{model_name}/models_fold_{fold_idx + 1}"  # Adjust the path as needed
    results_save_path = f"fold_results/du_mu/{model_name}/results_fold_{fold_idx + 1}"  # Adjust the path as needed
