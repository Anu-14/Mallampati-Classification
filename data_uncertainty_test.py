...
# from data_generator import LivenessDataset
...
data_dir = "/DATA/data/mallampati_data/selected_images_class_wise"
...
# Dictionary to store data for each fold
data_dict = {}
...
for fold_idx, (train_idx, val_idx) in enumerate(kf.split(dataset, labels)):
    ...
    # Assign dataloaders to 'train' and 'val' keys in the data_dict
...
class AleatoricModel(nn.Module):
    def __init__(self, eff_model):
        ...
        #################################################################
        ################ To chnge according to 4 classes ################
        #################################################################
    ...
    def forward(self,x):
        ...
        #softmax_output = nn.Softmax(logits)
...
class bayesian_categorical_crossentropy(nn.Module):
    ...
    def bayesian_categorical_crossentropy_internal(self, pred_var, true):
        ...
        undistorted_loss = self.categorical_crossentropy(pred+1e-15,true) 
    ...
    def gaussian_categorical_crossentropy(self, pred, true, dist, undistorted_loss, num_classes):
        ...
        distorted_loss = self.categorical_crossentropy(pred + 1e-15 + std_samples, true)
...
for T_monte_carlo in T_list:
    for fold in range(k_fold_splits):
        for loss_div in loss_list:
        ...
        if mode =='Train':
            ...
            for epoch in tqdm(range(num_epochs)):
                ...
                for images,labels in train_dataset:
                    ...
                    #print("### loss is ---->",loss,"cross",loss_crossentropy,"bay",loss_bayesian)
                    loss.backward()
                    optimizer.step()
                ...
                if f1v > best_f1:
                    ...
                    # torch.save(aleatoric_model,'july/models/.pt')
                # print(best_f1)
            ...
            filename =  f'models_output_du/Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}.pt'
            aleatoric_model=(torch.load(filename))
            aleatoric_model.train(False)
            aleatoric_model.eval()
            test_loss=0.0
            test_variance_loss=0.0
            test_crossentropy_loss=0.0
            test_acc=0.0
            test_bayesian_loss=0.0
            avg_test_loss=0.0
            avg_test_acc=0.0
            f1=0.0
            count_test = 0.0
            ...
            for batch_index, (images,labels) in tqdm(enumerate(test_dataset)):
                images = images.to(device)
                labels = labels.to(device)
                ...
                test_loss += loss.data
                test_variance_loss += loss_bayesian
                test_crossentropy_loss += loss_crossentropy
            ...
            avg_variance_loss = test_variance_loss / count_test
            avg_crossentropy_loss = test_crossentropy_loss / count_test
            avg_test_acc = test_acc / count_test
            avg_variance = variance / count_test

            print("For Model -----> ",filename," Test Loss: ",avg_test_loss," Test Variance Loss: ",avg_variance_loss, " Test Crossentropy Loss: ",avg_crossentropy_loss,"Test Accuracy: ",avg_test_acc,"Test f1 score: ", f1)
            print(classification_report(true_labelse, pred_labelse, labels=[0,1,2,3],zero_division=0))
            print(confusion_matrix(true_labelse, pred_labelse, labels=[0,1,2,3]))

            results = {
                "Accuracy": avg_test_acc.item(),
                "F1 Score": f1,
                "Variance": avg_variance.item(),
            }

            csv_filename = f"test_result_du/results_____Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}/{model_name}_results.csv"
            
            txt_filename = f"test_result_du/results_____Model_origdata_lossratio_{loss_div[0]}_{loss_div[1]}_monte_carlo_{T_monte_carlo}_effnet_aleatoric_pytorch{model_name}______LR_{learning_rate}______Batch_{batch_size}______Epoch_{num_epochs}/{model_name}_results.txt"
            ...
            with open(txt_filename, 'w') as txt_file:
                txt_file.write("For Model -----> " + filename + "\n")
                txt_file.write("Test Loss: " + str(avg_test_loss.item()) + "\n")
                txt_file.write("Test Variance Loss: " + str(avg_variance_loss.item()) + "\n")
                txt_file.write("Test Crossentropy Loss: " + str(avg_crossentropy_loss.item()) + "\n")
                txt_file.write("Test Accuracy: " + str(avg_test_acc.item()) + "\n")
                txt_file.write("Test F1 Score: " + str(f1) + "\n")
                txt_file.write(classification_report(true_labelse, pred_labelse, labels=[0, 1, 2, 3], zero_division=0))
                txt_file.write("\n")
                txt_file.write(str(confusion_matrix(true_labelse, pred_labelse, labels=[0, 1, 2, 3])))
